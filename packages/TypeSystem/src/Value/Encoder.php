...
{
    /**
     * Unicode high surrogate block for 2-byte char sequences.
     *
     * Note:
     *  - [0xD800...0xDB7F] is the High Surrogates block
     *  - [0xDB80...0xDBFF] is the High Private Use Surrogates block
     *
     * @see https://www.compart.com/en/unicode/block/U+D800
     * @see https://www.compart.com/en/unicode/U+D800
     * @see https://www.compart.com/en/unicode/U+DB80
     *
     * @var int
     */
    private const UTF_HIGH_SURROGATE_BLOCK = [0xD800, 0xDBFF];

    /**
     * Unicode low surrogate block for 2-byte char sequences.
     *
     * @see https://www.compart.com/en/unicode/block/U+DC00
     *
     * @var int
     */
    private const UTF_LOW_SURROGATE_BLOCK = [0xDC00, 0xDFFF];
    ...
    {
        [$result, $prev, $stream] = ['', null, Iter::toTraversable($this->lexer->lex($value))];

        while ($stream->valid()) {
            $current = $stream->current();

            $result .= $this->map($current, $prev);
     ...
     * @param TokenInterface|Composite|null $prev
     * @return string
     */
    protected function map(TokenInterface $token, TokenInterface $prev = null): string
    {
        switch ($token->getName()) {
            ...
            case self::T_UNICODE_CHAR:
                ...
                $prefix = $prev instanceof Composite && $prev->getName() === self::T_UNICODE_CHAR
                    ? $prev[0]->getValue()
                    : null;

                return $this->unicode($token[0]->getValue(), $prefix);
     ...
     * @return string
     */
    private function unicode(string $code, string $prefix = null): string
    {
        try {
            switch (true) {
                case $this->isHighSurrogate($code):
                    if ($prefix !== null && $this->isHighSurrogate($prefix)) {
                        return self::INVALID_UNICODE_CHAR;
                    }

                    return '';

                case $this->isLowSurrogate($code):
                    if ($prefix !== null && $this->isHighSurrogate($prefix)) {
                        return $this->unpack($prefix . $code);
                    }

                    return self::INVALID_UNICODE_CHAR;

                default:
                    return $this->unpack($code);
            }
        } catch (\Throwable $e) {
            return self::INVALID_UNICODE_CHAR;
        }
     ...
     * @return bool
     */
    private function isHighSurrogate(string $code): bool
    {
        $id = \hexdec($code);

        return $id >= self::UTF_HIGH_SURROGATE_BLOCK[0] && $id <= self::UTF_HIGH_SURROGATE_BLOCK[1];
    }
     ...
     * @return bool
     */
    private function isLowSurrogate(string $code): bool
    {
        $id = \hexdec($code);

        return $id >= self::UTF_LOW_SURROGATE_BLOCK[0] && $id <= self::UTF_LOW_SURROGATE_BLOCK[1];
    }
     ...
     * @return string
     */
    private function unpack(string $code): string
    {
        try {
